...
socketServer.addGroup(
  (node) => {
    node.addListener(
      ...
      { middlewares: [new OrderMiddleware(4), new OrderMiddleware(5), new OrderMiddleware(6)] }
    )
  },
  { middlewares: [new OrderMiddleware(1), new OrderMiddleware(2), new OrderMiddleware(3)] }
)
...
test.group('SocketServer: Simple Socket', (group) => {
  ...
  test(`Return the correct data in event 1: Should return a orders`, async ({ assert }) => {
  ...
  test(`Return the correct data in event 2: Should return a orders`, async ({ assert }) => {
    const socketClient = await initSocket(SERVER_PORT)
  ...
  test(`Return the correct data in event 3: Should return a orders`, async ({ assert }) => {
    const socketClient = await initSocket(SERVER_PORT)
