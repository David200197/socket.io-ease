...
socketServer.addGroup(groupWithoutPrefixMiddleware, { middlewares: [PassMiddleware] })
socketServer.addGroup(groupWithoutPrefixNotPassMiddleware, { middlewares: [NotPassMiddleware] })
...
test.group('SocketServer: Group Socket', (group) => {
  ...
  test(`Return the correct data without a global prefix: Should return a message "${RES_MESSAGE}"`, async ({
    ...
    const serverResponse = listenSocket(EVENTS[GROUP1].TEST1.CLIENT, socketClient)
  ...
  test(`Pass all middlewares: Should return a message "${RES_MESSAGE}"`, async ({ assert }) => {
    const socketClient = await initSocket(SERVER_PORT)
    const serverResponse = listenSocket(EVENTS[GROUP1].TEST1.CLIENT, socketClient)
